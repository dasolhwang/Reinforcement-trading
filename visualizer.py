...
class Visualizer:
    ...
    def prepare(self, coin_chart):
        ...
        # 거래량 가시화
        x = np.arange(len(coin_chart))
        volume = np.array(coin_chart)[:, -1].tolist()
        self.axes[0].bar(x, volume, color='b', alpha=0.3)
        # ohlc란 open, high, low, close의 약자로 이 순서로된 2차원 배열
        ax = self.axes[0].twinx()
        ohlc = np.hstack((x.reshape(-1, 1), np.array(coin_chart)[:, 1:-1]))
    ...
    def plot(self, epoch_str=None, num_epoches=None, epsilon=None, 
            action_list=None, actions=None, num_coins=None,
        ...
        self.axes[1].plot(x, num_coins, '-k')  # 보유 주식 수 그리기
...
class Visualizer:
    ...
    def prepare(self, coin_chart):
        # 캔버스를 초기화하고 4개의 차트를 그릴 준비
        self.fig, self.axes = plt.subplots(nrows=2, ncols=1, facecolor='w', sharex=True)
        for ax in self.axes:
            # 보기 어려운 과학적 표기 비활성화
            ax.get_xaxis().get_major_formatter().set_scientific(False)
            ax.get_yaxis().get_major_formatter().set_scientific(False)
    ...
    def plot(self, epoch_str=None, num_epoches=None, epsilon=None, 
        ...
        # 차트 4. 포트폴리오 가치
        self.axes[1].axhline(initial_balance, linestyle='-', color='gray')
        ...
        for learning_idx, delayed_reward in learning:
            # 학습 위치를 초록색으로 그리기
            if delayed_reward > 0:
                self.axes[1].axvline(learning_idx, color='r', alpha=0.1)
            else:
                self.axes[1].axvline(learning_idx, color='b', alpha=0.1)
