...
def preprocess_hour(chart_data):
    prep_data = chart_data
    windows = [5, 10, 20, 60, 120]
    for window in windows:
        prep_data['close_ma{}'.format(window)] = prep_data['close'].rolling(window*24).mean()
        prep_data['volume_ma{}'.format(window)] = (
            prep_data['volume'].rolling(window*24).mean())
    return prep_data

def preprocess_min(chart_data):
    prep_data = chart_data
    windows = [5, 10, 20, 60, 120]
    for window in windows:
